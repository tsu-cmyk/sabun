...
// Two surrounding rows preserve pixelmatch's AA neighbour-of-neighbour checks.
// Inputs are zero-copy views; only a bounded RGBA strip is allocated.
function visitDiffMask(a, b, width, height, threshold, visit) {
  const rows = 256, halo = 2;
  const scratch = new Uint8Array(width * Math.min(height, rows + halo * 2) * 4);
  for (let y0 = 0; y0 < height; y0 += rows) {
    const y1 = Math.min(height, y0 + rows);
    const top = Math.max(0, y0 - halo), bottom = Math.min(height, y1 + halo);
    const start = top * width * 4, end = bottom * width * 4;
    const mask = scratch.subarray(0, end - start); mask.fill(0);
    pixelmatch(a.subarray(start, end), b.subarray(start, end), mask, width, bottom-top,
      {threshold, includeAA:false, diffMask:true});
    for (let y=y0; y<y1; y++) for (let x=0; x<width; x++) {
      if(mask[((y-top)*width+x)*4+3]) visit(x,y);
    }
  }
}
...
function opHighlight(a, b, width, height, threshold, minBlockPixels, emphasize, localAlign = true) {
  ...
  visitDiffMask(a,b,width,height,threshold,(x,y)=>{
    mask[y*width+x]=1;
    blocks[((y/REGION_BLOCK)|0)*bw+((x/REGION_BLOCK)|0)]++;
  });
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const i = y * width + x;
      const p = i * 4;
      if (mask[i] === 0 || blocks[rowBlock + ((x / REGION_BLOCK) | 0)] === 0) continue;
      count++;
...
function opRegions(a, b, width, height, threshold, minBlockPixels, localAlign = true) {
  ...
  discardSparseBlocks(blocks, minBlockPixels);
  // 各ブロックの画素数は既に集計・ノイズ除去済み。マスク全画素の再走査は不要。
  let count = 0;
  for (const blockCount of blocks) count += blockCount;
  const regions = count > 0 ? findRegions(blocks, bw, bh, width, height) : [];
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      if (Math.abs(ga - gb) / 4 > threshold) {
        const px = x * 2, py = y * 2;
        const count = ++blocks[((py / REGION_BLOCK) | 0) * bw + ((px / REGION_BLOCK) | 0)];
        // 存在判定なので、1ブロックが確定すれば残りの画素を読む必要はない。
  ...
  try {
    ...
    if (op === 'highlight') {
      ...
      if (params.includeRegions) {
        // 表示画像と領域枠で、同じ位置合わせと入力バッファを共有する。
        const aligned = params.localAlign !== false ? locallyAlignB(A, B, width, height) : { data: B, adjustedTiles: 0 };
        const out = opAbsDiff(A, aligned.data, width, height, false);
        const { count, regions } = opRegions(A, aligned.data, width, height, params.threshold ?? 0.02, params.minBlockPixels ?? 5, false);
        self.postMessage({ id, ok: true, buf: out.buffer, count, regions, adjustedTiles: aligned.adjustedTiles }, [out.buffer]);
      } else {
